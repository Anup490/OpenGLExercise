...
void main()
{
	...
	vec3 inverted_texture = vec3(1.0f) - texture(material.specular, texCoord).xyz;
	vec3 specular_light = inverted_texture * spec * light.specular;

	FragColor = texture(material.diffuse, texCoord) * vec4(ambient_light + direct_light + specular_light, 1.0f);
}